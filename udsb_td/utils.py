...
def broadcast(f, *args):
    return {k: f(*[arg[k] for arg in args]) for k in directions.values()}

def split_key(key):
    key, *rs = jax.random.split(key, 3)
    return key, {
        FORWARD: rs[0],
        BACKWARD: rs[1]
    }
...
def sample_state_space(key, trajs, k):
    points = trajs.at[k].get()
    stds = trajs.at[k].get().std(axis=0)

    grid_shape = (100, points.shape[1])

    points = points[:grid_shape[0],:] + jax.random.normal(key, (grid_shape[1],)) * .05 * stds
...
def print_logs(logs):
    print(f"EPOCH #{logs['epoch']} \t loss={logs['loss']:.3f} \t ipf_loss={logs['ipf_loss']:.3f} \t td_loss={logs['td_loss']:.3f}")
...
def divergence(key, F, t, x):
    ...
    rand_a = jax.random.rademacher(key, (n, r, d))
...
def birth_by_splitting(key, status, birth_mask, reference_pos):
    key, key_half_splitting = jax.random.split(key)
